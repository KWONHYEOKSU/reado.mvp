...
export default function BlockEditor({ blocks, onChange, uploadedImages = [] }: BlockEditorProps) {
  ...
  const addImageTargetRef = useRef<number | null>(null)

  const updateBlock = useCallback(
    (index: number, content: string) => {
      const next = blocks.map((b, i) => (i === index ? { ...b, content } : b))
      onChange(next)
    },
    [blocks, onChange]
  )
  ...
  const addTextBlock = useCallback(
    (afterIndex?: number) => {
      const newBlock: ManualBlock = { type: 'text', content: '' }
  ...
  const addImageBlockFromUploaded = useCallback(
    (image: UploadedImage, afterIndex?: number) => {
      const newBlock: ManualBlock = { type: 'image', content: image.previewUrl }
      if (afterIndex === undefined) {
        onChange([...blocks, newBlock])
      } else {
        const next = [...blocks]
      ...
      <div className="flex gap-2 pt-2">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            const newBlock: ManualBlock = { type: 'image', content: url }
            onChange([...blocks, newBlock])
            e.target.value = ''
          }}
        />
